...
class PolicyService {
    async create(dto, userId) {
        ...
        if (existing)
            throw new errors_1.ValidationError(`Policy "${dto.name}" already exists`);
    ...
    async findById(id) {
        ...
        if (!policy)
            throw new errors_1.NotFoundError('Policy', id);
        return this.mapResponse(policy);
    }
    async update(id, dto, userId) {
        ...
        if (dto.blockOnCritical !== undefined)
            data.blockOnCritical = dto.blockOnCritical;
        if (dto.blockOnHigh !== undefined)
            data.blockOnHigh = dto.blockOnHigh;
        if (dto.blockOnlyFixable !== undefined)
            data.blockOnlyFixable = dto.blockOnlyFixable;
        if (dto.maxHighCount !== undefined)
            data.maxHighCount = dto.maxHighCount;
        if (dto.maxMediumCount !== undefined)
            data.maxMediumCount = dto.maxMediumCount;
        if (dto.slaCriticalDays !== undefined)
            data.slaCriticalDays = dto.slaCriticalDays;
        if (dto.slaHighDays !== undefined)
            data.slaHighDays = dto.slaHighDays;
        if (dto.slaMediumDays !== undefined)
            data.slaMediumDays = dto.slaMediumDays;
        if (dto.registryPatterns !== undefined)
            data.registryPatterns = dto.registryPatterns;
    ...
    async delete(id, userId) {
        ...
        if (!policy)
            throw new errors_1.NotFoundError('Policy', id);
        if (policy.isDefault)
            throw new errors_1.ValidationError('Cannot delete the default policy');
    ...
    async evaluate(imageId, policyId) {
        const prisma = (0, prisma_1.getPrisma)();
        ...
        if (!lastScan)
            return { passed: true, reason: 'No completed scan found', blockingCVEs: [], policyName: '' };
        const policy = policyId
            ? await prisma.scanPolicy.findUnique({ where: { id: policyId } })
            : await prisma.scanPolicy.findFirst({ where: { isDefault: true } });
        if (!policy)
            return { passed: true, reason: 'No policy configured', blockingCVEs: [], policyName: '' };
        ...
        function isBlockable(v) {
            if (exceptedCves.has(v.vulnerabilityId))
                return false;
            if (policy.blockOnlyFixable)
    ...
    async evaluateVulnerabilities(vulns, policyId) {
        const prisma = (0, prisma_1.getPrisma)();
        const policy = policyId
            ? await prisma.scanPolicy.findUnique({ where: { id: policyId } })
            : await prisma.scanPolicy.findFirst({ where: { isDefault: true } });
