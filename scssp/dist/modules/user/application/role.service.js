...
class RoleService {
    async create(dto, actorId) {
        ...
        if (dto.permissions && dto.permissions.length > 0) {
            ...
            if (permCount !== dto.permissions.length)
                throw new errors_1.ValidationError('One or more permissions not found');
        }
    ...
    async findById(id) {
        ...
        if (!role)
            throw new errors_1.NotFoundError('Role', id);
        return this.mapRoleResponse(role);
    }
    async update(id, dto, actorId) {
        ...
        if (existing.isSystem)
            throw new errors_1.ValidationError('Cannot modify system roles');
        ...
        if (dto.name) {
            await prisma.role.update({ where: { id }, data: { name: dto.name, description: dto.description } });
        }
        const role = await prisma.role.findUnique({
            where: { id },
            include: { permissions: { include: { permission: true } } },
    ...
    async delete(id, actorId) {
        ...
        if (!role)
            throw new errors_1.NotFoundError('Role', id);
        if (role.isSystem)
            throw new errors_1.ValidationError('Cannot delete system roles');
        const userCount = await prisma.user.count({ where: { roleId: id } });
        if (userCount > 0)
            throw new errors_1.ValidationError('Cannot delete role with assigned users');
