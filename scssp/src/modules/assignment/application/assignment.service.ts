...
export class AssignmentService {
  async create(dto: CreateAssignmentDto, assignedById: string): Promise<AssignmentResponse> {
    ...
    if (!vuln) throw new NotFoundError('Vulnerability', dto.vulnerabilityId);

    const existing = await prisma.vulnerabilityAssignment.findUnique({ where: { vulnerabilityId: dto.vulnerabilityId } });
    if (existing) throw new ValidationError('This vulnerability already has an active assignment');

    const assignedTo = await prisma.user.findUnique({ where: { id: dto.assignedToId } });
    if (!assignedTo) throw new NotFoundError('User', dto.assignedToId);
  ...
  async findAll(filters: { status?: string; assigneeId?: string; breached?: boolean; page?: number; limit?: number }): Promise<{ items: AssignmentResponse[]; total: number }> {
    const prisma = getPrisma();
    const { status, assigneeId, breached, page = 1, limit = 20 } = filters;
    const where: Record<string, unknown> = {};
    if (status) where.status = status;
    if (assigneeId) where.assignedToId = assigneeId;
    if (breached) where.slaBreached = true;
  ...
  async findById(id: string): Promise<AssignmentResponse> {
    const prisma = getPrisma();
  ...
  async updateStatus(id: string, dto: UpdateAssignmentStatusDto, userId: string): Promise<AssignmentResponse> {
    const prisma = getPrisma();
  ...
  async processSlaBreaches(): Promise<number> {
    const prisma = getPrisma();
    const now = new Date();
  ...
  async autoResolveByScan(scanId: string): Promise<number> {
    const prisma = getPrisma();
  ...
  async getComplianceReport(dateFrom?: string, dateTo?: string): Promise<ComplianceReport> {
    ...
    const now = Date.now();
    const agingBuckets: Record<number, number> = {};
  ...
  private mapResponse(a: any): AssignmentResponse {
    return {
      id: a.id,
