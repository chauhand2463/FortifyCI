...
export class PostureService {
  async computeSnapshot(scanId: string): Promise<void> {
  ...
  async getOrgTrend(dateFrom?: string, dateTo?: string): Promise<OrgTrendPoint[]> {
    const prisma = getPrisma();
    ...
    const dailyMap = new Map<string, { score: number; critical: number; high: number; medium: number; low: number; count: number }>();
  ...
  async getImageHistory(imageId: string): Promise<PostureSnapshot[]> {
    const prisma = getPrisma();
    const snapshots = await prisma.postureSnapshot.findMany({
      where: { imageId },
  ...
  async getLeaderboard(): Promise<{ best: LeaderboardEntry[]; worst: LeaderboardEntry[]; mostImproved: LeaderboardEntry[] }> {
    const prisma = getPrisma();
    const now = new Date();
    ...
    for (const image of images) {
      ...
      if (avgScore > olderAvg + 5) trend = 'improving';
      else if (avgScore < olderAvg - 5) trend = 'declining';
      else trend = 'stable';
  ...
  async getWeeklyDigest(): Promise<WeeklyDigest> {
    ...
    const scoreTrend: 'up' | 'down' | 'stable' = orgScore > olderScore + 2 ? 'up' : orgScore < olderScore - 2 ? 'down' : 'stable';
  ...
  private async countSuppressedForImage(imageId: string, vulns: any[]): Promise<number> {
    const prisma = getPrisma();
...
export const postureService = new PostureService();
